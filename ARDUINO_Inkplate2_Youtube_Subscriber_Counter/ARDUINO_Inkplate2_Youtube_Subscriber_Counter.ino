...
long subscribers_count = 123;
char subscribers_count_string[10];



char channel_id[] = "UCWHHWV_tkafsHNZ65V64sNQ"; // To get the channel ID of the public channel, go to the wanted Youtube
...
void drawAll()
{
  int char_index;
    ...
    display.drawImage(inkplate2_youtube_counter_212x104px_ps, 0, 0, 212, 104); // draw background image

    // for every digit
    sprintf(subscribers_count_string, "%5ld", subscribers_count); // convert integer into string
    for (int i = 0; i < 5; i++) {
      // calculate the image index based on the current character
      if (subscribers_count_string[i] == 32) {// this is a space > turn it into faded zero
        char_index = 10;
      } else {
        char_index = subscribers_count_string[i] - 48; // digit
      }
      // draw digit
      display.drawImage(bitmaps_digits[char_index], 80 + (25 * i), 43, 25, 40); // draw the digit image
    }
